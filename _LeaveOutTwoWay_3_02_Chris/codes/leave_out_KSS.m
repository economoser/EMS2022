...
s=['-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*'];
disp(s);
% [n_movers,J] = evalc('pruning_unbal_v3_test(y,firmid,id,id_old,firmid_old,controls)')
[results,y,firmid,id,id_old,firmid_old,controls] = evalc('pruning_unbal_v3(y,firmid,id,id_old,firmid_old,controls)'); % Note: Variable 'results' contains the text output from the output captured (i.e., not printed to display) by -evalc()-.
...
if n_of_parameters==3
    ...
    disp(s);
%     s=['Explained Variance - Plugin (R2): ' num2str((sigma_2_psi_AKM+2*sigma_alpha_psi_AKM+sigma_2_alpha_AKM)/var_den)];
    s=['Explained Variance Share (R2) - Plugin: ' num2str(explained_var_AKM)]; % ADDED BY CHRIS, 01/15/2021
    disp(s);
end
s=['-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*'];
disp(s);
...
if n_of_parameters==3
    ...
    disp(s);
%     s=['Total Explained Variation: ' num2str((sigma2_psi+2*sigma_psi_alpha+sigma2_alpha)/var_den)];
    s=['Explained Variance Share (R2) - Leave-Out: ' num2str(explained_var_leave_out)]; % ADDED BY CHRIS, 01/15/2021
    disp(s);
end
